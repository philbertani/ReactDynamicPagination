...
import {USD, useWindowSize} from "./utils"
...
const Paginated = (props) => {
  ...
  useEffect(()=>{
    let newItems = []
    for (let i=0; i<100; i++) {
        newItems.push({id:i, name:i, description:i, price: Math.random()*20  })
    }
    setItems(newItems)
      ...
      for (let i=itemStart; i<itemEnd; i++) {
        ...
        newJsxOutput.push(
          ...
          >
            ...
            <h3 style={{width:"100%",lineHeight:'30px'}} className="outlined">{item.name}</h3>
            <h4
              style={{
                lineHeight: "22px",
